...
import { titleCapitalised, urlWithRefFoodbank } from "@givefood/models";
import { timesince } from "../../lib/timesince";
import { adminPageContext } from "./pageContext";
import { adminProxy } from "./proxy";
import { adminGmapProxy } from "./gmapProxy";
import { adminSettings } from "./settings";
import { adminNeedDetail, adminNeedPublish, adminNeedUnpublish, adminNeedNonpertinent, adminNeedDelete, adminNeedsDeleteAll, adminNeedCategorise, adminNeedTranslations, adminNeedEditForm, adminNeedNotify } from "./needs";
...
// urls/needs.py:8 need_notify -- the email channel of it, see needs.ts.
adminApp.post("/need/:id/notify/", adminNeedNotify);
...
export async function adminIndex(c: Context<AppEnv>): Promise<Response> {
  ...
  const html = await render("admin/index.njk", {
    ...
    // :34-52). Both helpers were already ported in @givefood/models and applied on
