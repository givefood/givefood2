...
type JobMessage = { type: "media-backfill"; key: string } | TranslateNeedMessage | { type: "order-lines"; jobId: string; orderRowId: number } | { type: string; [k: string]: unknown };
...
async function dispatch(body: JobMessage, env: Env): Promise<void> {
  switch (body.type) {
    ...
    case "order-lines": {
      ...
      // SELF-RECORDING: the handler writes success or failure onto the
      // admin_job row and never throws, so a failed parse is a result the
      // order page shows rather than a message Cloudflare Queues retries into
      // the same paid Gemini call. "foodbank-check" used to sit beside this
      // case with the same contract; github #38 took it off the queue
      // entirely -- the check now runs inline in workers/site's admin route,
      // because ~30 s of its measured 37-40 s was this queue's own
      // max_batch_timeout waiting for a batch that never fills.
