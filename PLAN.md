...
| **Charity Commission (E&W, NI), OSCR, `members-api.parliament.uk`, `openpostcodes.uk`, `api.bankthefood.org`, `mapit.mysociety.org`** | Public/third-party APIs, called from queue consumers — except the postcode lookup, which is on the request path for `/needs/in/constituencies/?postcode=` and `/write/?postcode=`. **`openpostcodes.uk` replaced `api.postcodes.io` on 2026-09-09 (github #56)**; the rows above and in §2 that still name postcodes.io describe DJANGO, which is what they are for. |
...
| R6 | `parlcon.json` (2.6 MB) served from R2, gzipped, content-hashed. Postcode → constituency lookup keeps using an external postcode API but should move to the D1 `postcode` table once §04 lands — one indexed lookup, no external call. **⚠️ PARTLY SUPERSEDED 2026-09-09 (github #56): the provider is now `openpostcodes.uk`, not `api.postcodes.io`. The external call itself is still there** — what changed is that its answer is resolved against D1 by ONS code rather than by name (see R7). Moving the lookup off the network entirely remains open. | `/write/` renders; the map loads. | 0.5 |
| R7 | ~~**Do not reimplement `slugify`.**~~ **✅ RESOLVED 2026-09-09 (github #56).** `views.py:24` derived the constituency URL by slugifying the postcodes.io name and matching it against the stored `slug`; a JS slugify with different Unicode handling silently 404s constituencies with apostrophes, accents or ampersands. Derive-then-redirect is gone from both call sites (`wfbn/constituencies.ts`, `write/index.ts`): `openpostcodes.uk` returns `constituency.code`, and `getConstituencySlugByPcon24cd` resolves it against D1. The redirect target is therefore a row that has been READ, so it can no longer be a 404. | ~~Replace derive-then-redirect with a D1 lookup on the ONS `PCON24CD` code~~ — done. **The pre-cutover diff was run**: the port's slugify agreed with all 650 stored slugs (0 mismatches), so this replaced a mechanism that was correct and undetectably fragile rather than one already broken. `pcon24cd` is populated on 650 of 650 rows, none missing, verified read-only against production D1. | 0.5 |
...
| **D** | ~~Do 21 language variants of the 26 place sitemaps earn their place?~~ **RESOLVED 2026-08-30**, superseded by the broader 4-language decision at §2.7.1: place sitemaps exist in `en`/`cy`/`ga`/`gd` only, not 21. **Superseded again 2026-08-31/09-01, maintainer decision (workers/site/src/index.ts, the `/needs/at/place/` and sitemap comments): the browse-by-place page and `sitemap_places*.xml` are NOT built in any language -- out of scope, not deferred. This row's "4 languages" wording was stale by the time it was read during the 2026-09-05 launch audit; the code is authoritative.** Crawl-surface arithmetic still needs re-deriving for 4 languages, not 21 (§2.7.1's "not re-derived" note) — smaller than either the original 21-language number or the English-only floor this row used to recommend, since `cy`/`ga`/`gd` are real (if much smaller) advertised surfaces, not zero. | — |
...
| 6.4 | 🟡 **Core done 2026-09-02** (index, need detail, publish/unpublish, nonpertinent, delete/delete-all, categorise, translation fan-out, discrepancy actions — see note below); **notify split out as 6.4b, built 2026-09-05**. **Need-review queue first within the phase**: index, `/admin/need/<uuid>/`, the four transitions (publish / nonpertinent / categorise / notify), discrepancy actions. Most operationally critical workflow; give it the most build/QA time against the disposable D1 copy (§10.1.1a) before anything in Phase 6 goes near real production at launch. | A need is reviewed, published (fanning out 19 translations), categorised and notified end to end against the disposable D1 copy, matching Django's output for the same input. | 8 |
| 6.4b | ✅ **DONE 2026-09-05.** All four of the "notify" transition's send channels now go out: Postmark (per-subscriber email, broadcast stream + RFC 8058 one-click unsubscribe), Firebase Cloud Messaging, VAPID web push and the WhatsApp Graph API template send. Each is its own queue message from one `sendBatch`, so a channel whose credentials have expired cannot stop the other three — matching Django, whose four are separate tasks. **Both blockers named in this row's original text are gone.** The missing `whatsappsubscriber` table is created by migration 0020 and loaded (51 rows); while building it, all four subscriber tables turned out to be absent from `tools/pg-to-d1/extract_core.py`'s table lists entirely — launching before that fix would have silently dropped 202 subscribers, and an empty table looks exactly like a channel nobody uses. RFC 8291's message encryption is hand-implemented in WebCrypto as this row anticipated, and the "real risk of getting subtly wrong" is met head-on: a wrong implementation does not throw — it produces a well-formed message every browser silently discards while the push service still answers 201 — so `encryptPayload()` is split, with all randomness in the outer function and `encryptWithMaterial()` beneath it a pure function of its inputs. That makes **RFC 8291 §5's own published test vector** runnable against the real shipping code rather than a re-implementation of it: `pnpm run verify:webpush` (tools/webpush-vector/) reproduces the RFC's bytes exactly. VAPID_PRIVATE_KEY is accepted in both formats pywebpush accepts (PKCS#8 PEM, or a raw 32-byte scalar paired with VAPID_PUBLIC_KEY), since which one production holds cannot be read from outside a Worker secret. Firebase uses FCM v1 with an RS256 service-account assertion exchanged for an OAuth2 token — it addresses the topic `foodbank-<uuid>`, so `mobilesubscriber` is deliberately not read (that table is the /mobsub/ analytics record, not a send list). `need_email` (notification HTML/TXT preview) shipped earlier alongside the email channel. **Not test-sent:** every channel reaches real subscribers irreversibly, so pressing Notify is the maintainer's call. | Four channels send for real; a simulated 404/410 web-push response deletes the stale subscription. | 5 |
| 6.5 | 🟡 **Core done 2026-09-02** (9 of the real 18 `ModelForm`s — see note below for the exact count and what's deferred as 6.5b). Forms: hand-written HTML for ~20 models. `FOODBANK_FIELD_ORDER` is load-bearing and test-pinned. **Flag for approval:** the five near-identical `Foodbank` partial forms each repeat the same six-line `save()` stamping `edited=timezone.now()` and could collapse into one parameterised form. | Field order preserved. | 10 |
| 6.5b | ✅ **DONE — this row is stale, verified 2026-09-05.** Every blocker it names has since been removed by other work, and the forms were built without this row being updated. The three missing D1 tables all exist now (`orderitem` 0014, `ordergroup` 0015, `slugredirect` 0016), and `slugredirect` has a public-site read path too (middleware/slugRedirect.ts), which was the *more important* of the two reasons SlugRedirectForm was held back. Wired and reachable in workers/site/src/routes/admin/index.ts: order new/edit (orderForm.ts, `/admin/order/new/` + `/admin/order/<id>/edit/`), order groups list/new/detail/edit (orderGroup.ts), slug redirects list/new/edit (slugRedirect.ts). **Still not built, deliberately: `PlaceForm`** — for the third, unrelated reason this row already gave, which still holds: D1's `place` table (0009_aac.sql) was trimmed per §4.8.7 to "the only columns any production code path reads", so a faithful PlaceForm needs columns that do not exist, and un-trimming the schema was never asked for. `/admin/places/` is a read-only list (WP 6.9). Low launch risk either way: Django's own place rows are written by the loaders, not by hand. | Order/OrderGroup/SlugRedirect editable, `ordergroup`/`slugredirect` tables exist. | 4 |
| 6.6 | 🟡 **Core done 2026-09-02** (see note below for exactly what — 6 list views, 4 CSV exports, Foodbank create+cascade-delete, Location/DonationPoint delete). CRUD for remaining models; list views; CSV exports (column orders are pasted into spreadsheets — frozen). | 12 |
| 6.7 | 🟡 **Core done 2026-09-02** (detail page + 5 of 6 lazy tabs, touch, article toggle, crawl-set JSON — see note below for what's deferred and why). htmx surface: lazy tabs (`admin/foodbank.html#<tab>` partials → separate templates, same URL, same attributes), touch, use-ai, article toggle, delete. `/admin/crawl-set/<id>.json` shape is test-pinned. | Browser contract unchanged. | 5 |
| 6.8 | 🟡 **Core done 2026-09-02** (`foodbank_check`'s Queue+poll rebuild, `foodbank_use_ai_detail`, both debug views merged into `?debug=`; `needtestbed` deferred -- see note below). `foodbank_check` (6 fetches + Gemini) and `needtestbed` (14 sequential OpenRouter calls) → Queue + polling. Neither survives a request-scoped Worker. | The reviewer sees progress, not a hung tab. | 4 |
| 6.9 | ✅ **DONE 2026-09-02** (see note below). Rewrite `DISTINCT ON` at `gfadmin/views.py:3101` as `ROW_NUMBER() OVER (PARTITION BY ...)`. Fix `/admin/subscriptions/` and `/admin/places/` — both load whole tables into memory and will exceed 128 MB. | Real SQL pagination. | 3 |
| 6.10 | ✅ **DONE 2026-09-02** (see note below). **Admin query console** (see below). | Read-only, session-gated, `EXPLAIN QUERY PLAN` shown. | 1 |
| 6.11 | ⏸️ **Deferred to Phase 8, 2026-09-02** (see note below) — not a Phase 6 build task. Delete `gfauth`, `LoginRequiredAccess`, `OfflineKeyCheck`, `gfadmin/locale/` (20 dirs translating one empty msgid). | Dead code gone. | 1 |
...
> **WP 6.5's real form count, and what "9 of 18" covers** (research + build, 2026-09-02): `givefood/forms.py` defines 18 `ModelForm` subclasses, not the "~20" this row and PLAN.md's other counts have said twice — 13 of them really are `fields = "__all__"`, matching that half of the existing figure. `GfCredentialForm` (the 18th form beyond this WP's 17) isn't ported at all, same reasoning as WP 6.3's credential-view deletion: no D1-backed credential value exists for it to edit. `NeedLineForm` is not a separate build here — WP 6.4's `need_categorise` flow already covers what it does (and does it with a real `<select>` of categories rather than Django's hidden-type/free-text fields). **Built**: `FoodbankForm` (full, 30 fields, `edited` stamped), `FoodbankPoliticsForm` (same 30 fields, `edited` **not** stamped — maintainer decision, matching Django exactly rather than "fixing" the asymmetry), the 4 collapsed partials (Address/Phone/Email/FsaId — maintainer decision to collapse only these, not `FoodbankUrlsForm`), `FoodbankUrlsForm` (boilerplate edit/save only — its live-scrape-plus-Gemini-suggestion GET behaviour is deferred to sit alongside WP 6.8's Queue+polling infrastructure, the same category of work as `foodbank_check`/`needtestbed`), `FoodbankLocationForm`, `FoodbankDonationPointForm`, `ParliamentaryConstituencyForm`, and `NeedForm` (edit-only — its "new" URL is dead in Django too, nothing links to it; once `exclude` and every `editable=False` field are accounted for it only ever exposed 4 fields: `foodbank`, `change_text`, `excess_change_text`, `published`). **A real, disclosed gap found while building `FoodbankLocationForm`/`FoodbankDonationPointForm`**: neither Django's form nor its view nor any model `save()`/signal ever sets the denormalized `foodbank_name`/`foodbank_slug`/`foodbank_network`/`slug`/`company_slug` columns (all `editable=False`) — a location or donation point created through the real Django admin today would be unreachable by its own public detail URL. Populated here from the parent food bank instead of ported blank; `country` (NOT NULL in D1, unlike Django's looser `editable=False`-only declaration) defaults to the parent's own country on create. True geocoding-derived fields (county/district/ward/lsoa/msoa, `parliamentary_constituency*`, `mp*`) are left untouched either way — deriving those needs boundary-lookup infrastructure this port doesn't have.

> **WP 6.6, what's built and what's deferred** (research + build, 2026-09-02). 23 list views exist in Django (full inventory in that pass's own notes); this pass built the ones that matter most for the CRUD loop this phase already has edit forms for, all with **real SQL pagination even where Django's own view has none at all** — confirmed unbounded-in-Django: orders, order_groups, locations, donationpoints, items, politics, credentials, slug_redirects, foodbanks_without_need. Building every list page paginated from the start avoids shipping the antipattern once just to redo it in WP 6.9, which already names `/admin/subscriptions/` and `/admin/places/` as needing "real SQL pagination" — those two are deliberately left for 6.9 to build once, correctly (Django's own `/admin/subscriptions/` is worse than unpaginated: it fully materializes and Python-sorts 4 separate subscriber tables on *every* page view before paginating the in-memory result). **Built**: foodbanks list (open only, matching Django's `exclude(is_closed=True)`) + CSV (all, closed included — frozen columns: name/postcode/charity_number/country/last_order/last_need/no_locations/network/closed/url/created/modified), locations list, donation points list, politics/parlcon list + CSV (frozen columns: constituency/mp/mp_party/mp_parl_id — exports the *denormalized* copies cached on Foodbank/FoodbankLocation, not the ParliamentaryConstituency table itself, matching Django's genuinely odd but frozen contract), orders list + CSV (frozen columns: id/created/delivery/delivery_provider/foodbank/country/weight/calories/items/cost/delivered_cost) — read-only, since WP 6.5b's Order.save()-is-AI-dependent blocker only applies to create/edit, not to browsing rows the `orders`/`orderline` tables already hold — and needs CSV (frozen columns: id/created/foodbank/needs/excess/input_method; unbounded/unfiltered, unlike WP 6.4's review queue). Foodbank create (real, reachable in Django via a "New Foodbank" button, not dead like `NeedForm`'s) and cascade delete (12 related tables cleaned up in one atomic D1 batch, matching `Foodbank.delete()`'s manual cascade since every child FK is `on_delete=DO_NOTHING`; orders are unassigned not deleted, matching Django) — verified end-to-end including that Orders correctly survive with `foodbank_id` nulled. Location/DonationPoint delete. Unlike Django, every CSV response sets `Content-Disposition: attachment` (Django's 4 exports set none, so they render inline in-browser instead of downloading — a real, disclosed fix, not a port).
>
> **Deferred, not built this pass**: subscriptions list and places list (WP 6.9's own scope, named above). Items/OrderGroups/SlugRedirects lists (same D1-table gaps as WP 6.5b — no `orderitem`, `ordergroup`, or `slugredirect` table exists). `foodbanks_dupe_postcodes` (needs a full-table 1-hour-cache utility not built). Cross-model `/search/` (nice-to-have, not core CRUD). Crawl sets / crawl set detail (pairs better with WP 6.10's query console). `GfCredential` list is not a gap — WP 6.3 already decided credentials aren't ported at all.

> **WP 6.7, what's built and what's deferred** (research + build, 2026-09-02). This is the first WP to give the food bank a real tabbed detail page at all (`/admin/foodbank/<slug>/`) — every prior WP's forms redirected to it as "the closest thing that exists" (WP 6.5/6.6's own notes); those redirects now point at the real page. Built: the eager "general" panel (basic info, locations table, latest published need — Django toggles these three together under one `generallocations` class, not three separate tabs, reproduced exactly), and 5 of the 6 lazy tabs (needsorders, donationpoints, articles, subscribers, crawls), each its own `.njk` fragment reached via `GET /admin/foodbank/<slug>/tab/<tab>/` and the same hand-rolled `showtab once` htmx trigger WP 6.4's need-diff tabs already established a precedent for reusing `tabber.js` for — confirmed this is the actual mechanism (a custom DOM event dispatched by class-toggling JS, not any built-in htmx polling/revealed trigger). `foodbank_touch` (bumps `edited` only, htmx → a literal disabled-button fragment, plain POST → redirects to the foodbanks **list**, not back to the detail page — matched exactly, it's easy to get backwards). Article `featured` toggle — corrected from this WP's own framing: it lives on the **admin dashboard**, not `foodbank.html` (confirmed by reading the template; WP 6.4's dashboard gained a "Recent articles" panel to host it). `/admin/crawl-set/<id>.json`, matching the test-pinned shape exactly, built standalone even though its own consumer page is deferred (below) — PLAN.md names the JSON endpoint as its own deliverable regardless.
>
> **Deferred, not built this pass, both reasons genuine blockers not scheduling**: the **photos** tab and `photo_delete` — Django's `PlacePhoto` is a real queryable model (`id`, `place_id`, ownership-checked against a food bank's own/its locations'/its donation points' `place_id`s before allowing delete); this port's photo architecture (`workers/site/src/routes/media.ts`) has no D1-backed photo metadata at all, by design — `serveMedia()` is a bare `R2.get()` keyed by URL path, no lookup. Building a photos tab needs a new table and a real design decision about what it tracks, not just a template. **use-ai** (`foodbank_use_ai_detail`) — confirmed it makes no AI call itself, it only *commits* a value `foodbank_check` already found; its only caller is `check.html`, which doesn't exist yet and depends on a synchronous Gemini call inside a GET handler — exactly WP 6.8's "Neither survives a request-scoped Worker" territory. Building the commit-only endpoint without the page that produces values to commit has no real use, so both wait for WP 6.8. The full `crawl_set.html` polling page (plain `setInterval`+`fetch`, **not** htmx, confirmed by reading the actual script) stays bundled with WP 6.6's crawl-sets-list deferral — only the JSON endpoint it would poll is built here.
>
> **A real bug found and fixed during verification, not by inspection**: the crawl-set JSON endpoint's "not found" path first used the framework's own 404 dispatch (`c.notFound()`), which — for every OTHER admin URL, all of which end in `/` — correctly triggers index.ts's ported `APPEND_SLASH` middleware (probe the URL with a trailing slash appended; redirect if that resolves). Django's own `crawl-set/<id>.json` pattern deliberately has no trailing slash (it's a file-suffixed resource, not a directory-style page), so that same global probe fired incorrectly here, turning a legitimate 404 into a wrong 301. Fixed by returning a plain 404 `Response` directly for this one route, bypassing the global handler — the only admin URL in this phase without a trailing slash, so the only one exposed to it.

> **WP 6.4/6.7 fidelity follow-up, 2026-09-02** — a real gap found by the maintainer live, not by inspection: the dashboard and foodbank detail page had been marked "core done," but each rendered only a fraction of Django's actual content. Closed both, plus the three related admin-triggered crawl actions:
>
> - **Dashboard** (`admin/index.njk`): rebuilt as Django's real 3-column layout; added the whole missing "Metrics" panel (oldest/latest edited foodbank, 24h need/article/charity check counts, 24h published need-line count, oldest/latest need-check foodbank) via a new `getAdminDashboardStats()` query. `tasks_24h`/`tasks_outstanding` are disclosed as not portable — Django reads them from `django_tasks_db.DBTaskResult`, which Cloudflare Queues has no equivalent of. Discrepancies regained inline done/invalid actions and foodbank links; both need tables and discrepancies gained `timesince` formatting.
> - **Foodbank detail page** (`foodbank_detail.njk`): the General panel had 4 fields; Django's has six more sections (URLs/Info/Contacts/Politics/Stats/Dates) and ~30 fields, plus tab icons/count badges and a Delete button. D1 already had the columns; this was templating and one new aggregate query (`getFoodbankAdminTotals`, matching Django's own "every tab count and order total, in one query" framing).
> - **Force Check / Force Article Crawl / Force Charity Crawl**: genuinely new admin-triggered single-foodbank actions, not a template gap. Reuse the SAME queues (`needcheck-render`/`articles`/`charity-{ew,scotland,ni}`) `workers/jobs`'s own cron fan-out (`scheduled/index.ts`) already produces to and consumes from — a one-row `CrawlSet` (no `run_id`, admin-triggered) plus a single queue message, instead of the cron's whole-table fan-out. `workers/site/wrangler.jsonc` gained producer bindings for all five queues (a queue can have multiple producer Workers; the consumer stays on `workers/jobs` only). The three message-shape interfaces are duplicated in `workers/site` rather than imported from `workers/jobs/src` — the two Workers never share `src/`, same as every other cross-Worker duplication already in this codebase (`BOT_USER_AGENT`, `slugify`).
> - Explicitly **out of scope, maintainer decision**: RFI (`foodbank_rfi`) and "Resave Everything" (`foodbank_resave`) — the latter would need every implicit Django `.save()` side effect (geocode, decache, parliamentary-constituency lookup) identified and replicated, real new design work most similar to a WP that was never scoped, not a template gap.
> - **A real bug found and fixed along the way, not by inspection**: `timesince()` (`workers/site/src/lib/timesince.ts`) produced "NaN years, NaN months ago" for any date with a trailing "Z" — which is every app-written D1 timestamp (`new Date().toISOString()`), just not the initial bulk-migrated data that happened to mask it until now. `parseUtc()`'s fractional-seconds split never stripped the "Z" first, so it either contaminated the millisecond digits or, with no fractional seconds at all, the seconds field itself. Fixed in `parseUtc()`; affects five other pre-existing call sites too (`frag.ts`'s public "last updated" fragment, `api1.ts`, three dashboards routes) that were silently exposed to the same defect going forward.
> - Verified end-to-end against local D1 with hand-seeded data covering every new field, count, and action (including CSRF enforcement and both crawl actions' guard conditions correctly no-op-ing when `rss_url`/`charity_number` are absent).

> **Full admin fidelity audit and fix pass, 2026-09-02** — the maintainer flagged "the admin is still missing plenty" after the dashboard/foodbank-detail fix above; rather than guess which page, a 7-way parallel research pass compared every remaining admin page against its Django original, found ~40 gaps (13 high-severity), and every high-severity one was closed same-session:
>
> - **Need workflow**: restored the "Edit" row on the need detail page's Workflow table (the route/template already existed, just unreachable); Food Bank name links on both need.njk and need_translations.njk; short (7-char) IDs everywhere instead of the full uuid; the input-method emoji; Django's relative `timesince` formatting for every timestamp (Created/Modified/Previous-published/nonpertinent/Crawl Set start) — `need_translations.njk` was also missing Modified entirely.
> - **Discrepancy page**: restored the real three-column "fix it" workflow PLAN.md itself names as required — an embedded, editable 30-field foodbank form (reusing `foodbank_form.njk`'s own fieldset macro) that posts to `foodbank_edit?discrepancy=<id>`, which both saves the foodbank and auto-resolves the discrepancy in one request. The code comment justifying the missing column ("foodbank edit forms don't exist yet, WP 6.5") had gone stale — that form shipped and nobody circled back.
> - **5 list pages** (foodbanks/locations/donation points/politics/orders): each showed 3-5 columns where Django shows 9-13, mostly because the row data was already fetched (`SELECT *`) and never rendered — the same root cause as the dashboard bug. Foodbanks gained a genuinely new correlated-subquery aggregate (28-day hits); politics's own query was widened (was hardcoded to 4 columns). Orders list gained real sorting (was hardcoded to one order with no UI to change it) and links to a new order detail page.
> - **Foodbank detail page's 4 lazy tabs**: needs table was missing the need's actual content (`change_text`) entirely; orders table and donation points table were both missing most columns and, for orders, any destination page to link to (built one — read-only, matching Django's own `order()` view, explicitly not `order_edit`/`delete`/`send_notification`, same out-of-scope category as RFI/Resave above); subscribers tab was three separate silently-20-row-capped lists instead of Django's one combined, unbounded, chronologically-sorted table; crawls tab was missing the real finish time and duration. Tab row caps corrected to Django's real 200/100 (were an invented 20 for all three).
> - **A second instance of the same `timesince.ts` bug class**, found while fixing the crawls tab: `getCrawlSetJson`'s own duration math had the identical "append a second Z to an already-Z-suffixed timestamp" defect, silently producing `NaN` for every finished crawl's `time_taken`/`time_taken_ms`. Extracted a shared `parseD1Timestamp()` helper, fixed both the pre-existing call site and the new one.
> - **Form validation**: `parseAdminFields()` — shared by every foodbank/location/donation-point create/edit form — had no format checking at all, only required-and-non-empty. Added postcode (Django's own regex, case-insensitive) and email format validation, the two fields PLAN.md's own pre-build research names as "the only validation on some fields."
> - **Subscriptions list**: the type-filter dropdown (`?type=`) was fully supported server-side but had no UI control at all.
> - **Check page**: built "Next" (`foodbanks_next` — redirect to the oldest-edited open foodbank, working the review queue one foodbank at a time), which didn't exist anywhere. Fixed the Details table's Address row to combine address+postcode for comparison exactly as Django does (was silently never highlighting, and dropping postcode from the comparison entirely) — the underlying job-consumer bug lived in `workers/jobs`, not the template. Added street addresses to the location/donation-point comparison lists (present in the AI response, never rendered; never even fetched for "ours" items).
> - **Sign-in page**: `/auth/` always 302'd straight to Google unconditionally, including for an already-signed-in visitor — no branded page ever existed. Built one (signed-out: a button starting the existing PKCE flow; signed-in: profile + sign-out), without touching the tested gate-redirect path (`?next=`, set only by `requireAdminAuth`, still goes straight to Google). A rejected sign-in also used to bare-403; now redirects back to the sign-in page instead of dead-ending.
> - **"New Location Area"** (MapIt): entirely unported — no route, template, or button. Creates a `FoodbankLocation` from a MapIt administrative area's centroid + boundary geometry instead of a postcode, reusing `upsertLocation` (WP 6.5) directly. PLAN.md's own research calls this form's 6 distinct error-reporting paths "the template for how every other form should report failure" — ported one for one. Needs a new secret, `MAPIT_KEY` (already named in PLAN.md §10.2.7's own table) — added to the type stub only, **not** to `wrangler.jsonc`'s `secrets.required`, pending the maintainer confirming it's set on the real account.
> - Explicitly deferred (medium/low severity, not attempted this pass): per-row redisplay-with-data on form validation errors (every admin form still bare-`c.text()`s a 400 instead of re-rendering with the error inline); ascending sort options on several lists (Django offers both directions, the port is DESC-only); a few cosmetic gaps (delivery-provider/crawl-type icons already added where they mattered functionally; some raw-field-key labels on the check page).
> - Verified end-to-end against local D1 for every item above, including a real live call to `mapit.mysociety.org` for the New Location Area error path.

> **WP 6.8, what's built and what's deferred** (research + build, 2026-09-02). PLAN.md's own §9.4.4 already designed this WP's architecture in detail before any of it was built -- an `admin_job` table (id/kind/target/status/result/error/created/finished) plus enqueue-then-poll, explicitly **not** Workflows ("a Queue message plus an `admin_job` row gives the same user experience with a primitive the team already has"). Built exactly that, plus took §9.4.4's own S9 sub-recommendation: Django's `foodbank_check_prompt`/`foodbank_check_result` are two unlinked debug-only views that each **re-run the entire 5-6-fetch scrape plus a second Gemini call from scratch** just to dump the prompt text or raw JSON (confirmed by reading both — no caching, no shared state with the real `foodbank_check` view). Folded into `?debug=prompt`/`?debug=json` on the same check page instead, reading the *already-completed* job's stored result — zero extra fetches or AI calls, not a port of the waste. One correction to PLAN.md's own citation while implementing: §9.4.4 says the poll mechanism should reuse "the pattern already in the codebase (`hx-trigger="every 2s"`)" and points at `admin/index.html:117` as prior art — that line is not htmx at all, it's a hand-rolled `fetch()`+`setInterval()` via `csi.js`. Since this check page has no existing browser contract to preserve (unlike WP 6.4/6.7's tab mechanisms, which do), htmx's own built-in `hx-trigger="every 2s"` is used directly rather than porting `csi.js` for one new page.
>
> **Verified end-to-end against local D1** short of the live Gemini call itself (no key available to this build, and a real call spends real money): enqueue creates the `admin_job` row and redirects with `?job=`; the poll endpoint returns the in-progress fragment while queued/running and switches to `HX-Redirect` back to the check page once done or failed (confirmed both branches, including that a *stale* `created` timestamp format in hand-inserted test data — not a real code path — was the one time the "show the latest job" query looked wrong, not the query itself); the completed-results page renders `check-changed` highlighting and "Use" buttons correctly against a hand-seeded job result; `?debug=prompt`/`?debug=json` both read back correctly; `foodbank_use_ai_detail` was verified for real (phone-number space-stripping, `edited` stamping, and all three of Django's field/email/URL validation rejections, matching gfadmin/tests/test_use_ai_detail.py's own assertions). `GEMINI_API_KEY` is a new secret this WP needs (added to `workers/jobs/wrangler.jsonc`'s `required` list, consistent with every other name already there — that Worker has no Workers Build project connected yet, so the whole list is equally unverified) — not set on the real account, a disclosed gap exactly like WP 6.1's OAuth secrets were until the maintainer set them.
>
> **Deferred, documented, not built this pass**: `needtestbed` (14 sequential OpenRouter calls comparing model quality) — confirmed genuinely unreachable from any template or nav in Django (a bare URL a developer has to know and type, same category as `emailtester`/`webpushtester`), a pure internal eval tool for choosing which model production's need-check pipeline should use, not reviewer-facing. Building 14-way fan-out plus a comparison table for a zero-linkage debug tool is real scope with a separate `openrouter_needtestbed` credential of its own; the `admin_job` infrastructure this WP built makes adding it later straightforward (a `needtestbed` job `kind` on the same table, same poll mechanism) if wanted. `FoodbankUrlsForm`'s GET-time scrape+Gemini suggestion behaviour (deferred from WP 6.5 to "sit alongside this WP's Queue+polling infrastructure") stays deferred one more step -- the infrastructure now exists, but wiring it up is its own scoped follow-up, not bundled into this already-large pass. Two disclosed fidelity reductions inside `foodbank_check` itself: the 6th fetch (a POST special-cased to food banks using the shared "foodbank.org.uk donate-food" white-label donation-point finder) and the `network_id` auto-backfill regex-scrape that feeds it are not built — narrow to one third-party platform, and food banks on it get an incomplete donation-points section in their check result until picked up.

> **WP 6.9, what's built** (research + build, 2026-09-02). All three named items, fully built and verified against local D1 — no deferrals, unlike 6.5-6.8. **`DISTINCT ON`** at `gfadmin/views.py:3101` rewritten as `ROW_NUMBER() OVER (PARTITION BY foodbank_name ORDER BY created DESC)` in a subquery, LEFT JOINed back to `foodbank` (`getFoodbanksWithoutNeedPage` in `packages/db/src/adminLists.ts`) — SQLite has no `DISTINCT ON` at all, so this is the portable equivalent, not a stylistic choice. **`/admin/places/`** (`getPlacesPage`) — real `LIMIT`/`OFFSET` on `place` (id/name/county/population, the 3 sortable columns that survived §4.8.7's schema trim), replacing Django's `Paginator(all_places, 20000)`. **`/admin/subscriptions/`** (`getSubscriptionsPage`/`subscriptionUnionSql`) — Django's own view is worse than unpaginated: it fully materializes 4 separate subscriber tables and Python-sorts them **on every page view** before paginating the in-memory result. Rewritten as one `UNION ALL` (email/mobile/webpush branches, each JOINed to `foodbank` for the denormalized name/slug; "whatsapp" excluded as a `?type=` option — no D1 table exists for it, the same gap already disclosed elsewhere) with `ORDER BY created DESC LIMIT ? OFFSET ?` applied to the *outer* combined query, so D1 does the sort and page server-side instead of the Worker. `deleteSubscription` handles the three storage shapes: email rows are keyed by a composite `"<email>|<foodbank_slug>"` (no numeric id on that table), mobile/webpush by a plain numeric id.
>
> **A real bug found via code review, not by inspection, and fixed before any live test**: the first draft of `adminSubscriptionsList` called `adminPageContext()` once directly to get a `csrf_token` for building each row's delete-form HTML, then called `renderList()`, which internally called `adminPageContext()` **again**. `issueCsrfToken()` (`lib/csrf.ts`) mints a fresh raw token plus a new signed cookie on *every* call — only the second call's cookie reaches the browser, so every row's embedded token would have mismatched it and every delete would have failed CSRF verification. Fixed by refactoring `renderList<T>()` to call `adminPageContext()` exactly once and thread its `csrfToken` into a new `rowActions?: (row, csrfToken) => string` callback signature; removed the separate outer call. Verified fixed with a real cookie-jar round-trip against local D1 (delete succeeded, confirmed via a row-count check afterward) — not just re-reading the diff.
...
> **WP 6.10, what's built** (build, 2026-09-02). All four bullets above, plus one addition this spec's own sketch didn't cover: `GET /admin/query/` (`workers/site/src/routes/admin/query.ts`) renders the empty form; the query itself only ever runs on `POST /admin/query/`, matching "POST-only" literally — never accepted via a URL query string, so a stray link or `<img>` tag can't trigger one and raw SQL never lands in an access log. Genuinely new — there is no Django admin page like this to port, so the shape above is what got built, not a translation of an existing template. One safety addition beyond the spec's four bullets: rejects a semicolon anywhere before the trimmed end (blocks statement-stacking, e.g. `SELECT 1; DROP TABLE foodbank`) — cheap insurance alongside the `SELECT`/`EXPLAIN` prefix check, even though D1's `prepare()` already only executes a single statement. The 500-row cap is enforced by wrapping the admin's own query as `SELECT * FROM (<query>) LIMIT ?` with `500` **bound**, not interpolated — the one place this WP's own SQL text is built from more than the admin's raw input, so it's the one place "parameterised" actually applies; the wrapping subquery also means an admin-supplied `ORDER BY`/`LIMIT` inside their own query still works exactly as they wrote it, since the cap only clips the outer result set. `EXPLAIN`-prefixed input is run as-is with only the plan section shown (no results, no LIMIT — a plan has no row cap to speak of); plain `SELECT` input gets both an `EXPLAIN QUERY PLAN` run and a capped results run. D1 errors (bad SQL, missing table) are caught and shown verbatim in the page — safe and actively useful here, since this is an authenticated, admin-only diagnostic tool, the direct equivalent of a `psql` error at the prompt. Verified end-to-end against local D1 with a hand-seeded session cookie (no real Google OAuth flow available to a curl-based test): valid `SELECT` (plan + results), `EXPLAIN QUERY PLAN`-only input (plan only, no Results section — confirmed via an explicit `show_results` flag, not an accidental `rows.length` check that would have also hidden a legitimately empty result set), `DELETE`/`DROP`/`PRAGMA`/stacked-statement rejection, a genuine SQL error (`no such table`), a missing-CSRF-token 403, an unauthenticated redirect to `/auth/`, and a query with its own `ORDER BY`/`LIMIT` surviving the subquery wrap intact.
>
> Not linked from any nav or dashboard — `/admin/places/`, `/admin/subscriptions/`, and `/admin/foodbanks/without_need/` (WP 6.9) are the same, direct-URL-only, and this follows that precedent rather than introducing new admin-nav structure unprompted.

> **WP 6.11, rescoped 2026-09-02: this is a Phase 8 action, not a Phase 6 one.** `gfauth`, `LoginRequiredAccess`, `OfflineKeyCheck`, and `gfadmin/locale/` live only in the Django codebase (`foodcharity`), which this whole build has treated as a **read-only reference** — the migration's own phasing (§10.1.1a, this section's Phase 7/8 table) is explicit that nothing changes on the live Django app or the live zone before the single production cutover; Django keeps serving `www.givefood.org.uk` until then. There is nothing to delete in *this* repo — `givefood/` never contained a copy of Django's source, only the D1/Workers rebuild — so "done" for this WP is confirming the Workers-side replacements those four items need are ready, not deleting anything now:
>
> - `gfauth` → superseded by WP 6.1/6.2's `lib/adminAuth.ts` Authorization-Code+PKCE flow (done).
> - `LoginRequiredAccess` → its `email_verified && hd == "givefood.org.uk"` gate is reproduced verbatim inside that same flow, per `lib/adminAuth.ts`'s own top comment (done).
> - `OfflineKeyCheck` → `/offline/` stops being an HTTP surface entirely (§8.12); nothing in Phase 6 depends on it.
> - `gfadmin/locale/` → the Workers admin is English-only by design (no `gfadmin` template calls `{% load i18n %}` beyond what the *public* site's `packages/templates` i18n covers); nothing ports a translated admin string.
>
> All four are confirmed safe to delete **once Phase 8 actually runs** — folded into that phase's existing decommission scope (8.1/8.2) alongside the rest of `foodcharity`, rather than treated as a separate pre-cutover deletion this session would otherwise be tempted to do directly against a currently-live production app.

> **A live production 500 on `/admin/foodbanks/`, found and fixed 2026-09-02.** The maintainer reported the list 500ing on real beta traffic. Diagnosed against production, not guessed at from local data: a short-lived KV session seeded directly into the real `SESSIONS` namespace, then `wrangler tail` on the actual deployed Worker (`givefood2`, not the launch-config name) while re-triggering the request, caught the real stack trace — `timesince.ts`'s `parseUtc()` threw `TypeError: Cannot read properties of undefined (reading 'replace')` on a date-only D1 value (no space, no "T"): `s.split(/[ T]/)` returns a one-element array on that input, so `rawTimePart` is `undefined`, not `""`. Some stored dates genuinely have no time component; fixed by defaulting to `"00:00:00"` when the split doesn't find one. Deployed and confirmed live (200 OK, the real 1023-row count).

> **A second maintainer visual report — "`/admin/foodbank/faversham/` doesn't look like the live site" — traced to two distinct defects, both fixed same-session, 2026-09-02.** Verifying this one honestly needed a real authenticated screenshot of a real Worker response against real production data, not an HTML diff: a temporary local-only `/__diag_login__` route (issuing a genuine `Set-Cookie` — session cookies are `HttpOnly`, unreachable from JS) plus a temporary `"remote": true` on the D1 binding in `wrangler.jsonc` (both reverted before commit, confirmed via `git diff` showing no residual change to either file) let the Browser pane render Faversham's actual detail page through the local dev server. First finding attempt was a false positive worth recording: replaying a curl-fetched HTML snapshot as a static file made every MDI icon and custom font read as failed/`"error"` in `document.fonts` — a cross-origin `<base href>` artifact of *that* testing method, not a real bug, caught by comparing against a genuine navigation to `beta.givefood.org.uk` (fonts loaded fine there) before it was ever reported.
>
> - **Empty-`<dd>`-collapses-the-row.** `gf.css` floats `<dt>` left at a fixed width; an empty `<dd></dd>` (Faversham's null `last_rfi`) computes to 0px height, so the *next* floated `dt` renders on the same row instead of wrapping — confirmed via `getBoundingClientRect()` showing two `dt`s at the same y-coordinate. Fixed in `foodbank_detail.njk`'s Dates section (`Edited`/`Last Need`/`Last RFI`/`Last Discrepancy Check`/`Last Crawl`) and the Info/Politics sections' `Network`/`Parl. Con.` rows with a `&nbsp;` placeholder when the value is empty — preserving Django's own "always show the label" behaviour (no field is hidden) while giving every `<dd>` non-zero height.
> - **Raw D1 timestamps instead of Django's own default rendering — the dominant visual difference, and systemic, not local to this one page.** Django prints an unfiltered `{{ some_datetime }}` through `DATETIME_FORMAT` ("N j, Y, P", locale `en`, `USE_L10N=True` — confirmed straight from `django/conf/locale/en/formats.py`, not assumed), e.g. "Sept. 2, 2026, 3:34 p.m." Every port of a bare `{{ x.created }}`-style field had instead been passed the raw D1 string — down to the six-digit fractional seconds — because `djangoDate`/`formatDjangoDateTokens` (`packages/templates/src/filters.ts`) already existed for the *explicit* `|date("...")` call sites but was never reached for the *implicit* ones Django applies automatically. Added Django's `N` (AP-style month abbreviation — ported verbatim from `django.utils.dates.MONTHS_AP`: not `Sep.`/`Sept.` uniformly, `March`/`April`/`May`/`June`/`July` are spelled in full) and `S` (ordinal day suffix) tokens, and hardened `djangoDate` itself against the exact two D1-timestamp edge cases `timesince.ts`'s `parseUtc` already had to learn the hard way this session (a trailing "Z", a missing time component) rather than risk a fourth instance of that bug class. Exported `djangoDate` from `@givefood/templates` so the list-page row-builder (`workers/site/src/routes/admin/lists.ts`, plain TypeScript, not a template) could call the same function instead of a second implementation. Applied everywhere a raw date was rendering unformatted: `foodbank_detail.njk`'s Dates section and Latest Need link, `order.njk` (Created/Modified/Found, plus the Delivery row's missing day-of-week prefix and the Notification line's missing `timesince`), `index.njk`'s dashboard metrics and the Articles table's `"D jS H:i"` format, three `foodbank_tabs/*.njk` fragments (needsorders/subscribers/crawls), `discrepancy.njk`, and every list page's date columns in `lists.ts` (`dateCell`/new `plainDateCell`, matching each Django list template's own with/without-`timesince` pattern exactly rather than applying one pattern everywhere).
> - **`foodbank_detail.njk`'s "Latest Need" section** was also structurally wrong, found while fixing its date: the original shows the input-method emoji, a link to the need's short ID, and `timesince` text; the port showed the raw `created` timestamp as the link text with none of the rest. Rebuilt to match, enriching `foodbank.latestNeed` server-side (`need_id_short`/`input_method_emoji`/`created_timesince`) the same way `needsorders.njk`'s own enrichment already did.
>
> Deliberately left alone: `order.njk`'s missing Edit/Delete/Send-Notification/Order-group actions (a documented, pre-existing scope decision — `orderAdmin.ts`'s own comment, and the maintainer's own "we don't need the resaver"/"nor do we need the send RFI" scope cuts earlier this session — the order page is read-only by design) and `discrepancy.njk`'s three added Type/Status/Created columns that don't exist in the Django original (an addition, not a regression — left as-is, just given the same correct date formatting as everything else). Verified against real production data (Faversham, via the diagnostic route above) for the visual fix and via `tsx` for the exact token output (`Jan. 16, 2020, 2:03 p.m.`, `Sept. 2, 2026, midnight`, `Wed 2nd 15:30`, etc., matching Django's own formatter output) before any live check.

> **WP 6.12 — "build it like the old Django site": the whole-admin completeness pass, 2026-09-02.** After the date-formatting fix above, the maintainer's verdict was still "the admin of this site isn't good enough. keep going to build it like the old django site". So this pass stopped fixing individual pages and instead inventoried **every** `gfadmin` URL against the port: 113 Django URL patterns across `gfadmin/urls/*.py`, compared route by route. Roughly 30 had no port at all. Two parallel research/build passes (14 research agents reading the Django views and templates in full, then 9 build agents working from those specs) closed the gap.
>
> **The page shell was the most visible defect, and it was on every single page.** Django's navbar has seven items (Food Banks, Locations, Donation Points, Needs, Orders, Crawls, Settings) plus a site-wide search box; the port had four and no search. Three of the missing four pointed at pages that did not exist. Also missing from the shell: the mobile `navbar-burger` (so below Bulma's 1024px breakpoint the admin nav was simply unreachable), the footer, the font preloads, and the four Google-key globals.
>
> - **`static/js/admin.js` was entirely dead, and had been all along.** It is served identical to Django's copy but for a single line added later (the company auto-select listens on `input` rather than `keyup` -- github #57, because a pasted or autofilled donation point name fires no keystroke), and all six of its form initialisers select controls by the `id_<name>` ids Django's `{{ form|bulma }}` emits — which `includes/formfields.njk` never rendered. So "Get Lat/Lng & Place ID", "Lookup Location", "Lookup Donation Point", "Make Titlecase", "CSV to Lines" and "Find & Replace" were never even *injected* into any admin form, on any page. Two more layers of the same failure sat behind it: the script reads `gmap_geocode_key`/`gmap_places_key`/`gmap_static_key` as bare globals that nothing defined (`ReferenceError` on the first click), and it fetches `/admin/proxy/gmaps/{textsearch,placedetails}/`, which had no port. Fixed at all three layers: ids and `class="form-<slug>"` on the form templates, the globals in `adminPageContext`, and the proxy ported — with the Places key moved **server-side** (`routes/admin/gmapProxy.ts` injects it and discards any client-sent `key`), so unlike Django it never reaches the browser at all, and `admin.js` still works unchanged because its now-empty `key=` param is simply ignored.
> - **`version` was the literal string `"dev"`** on every admin page — `buildPageContext`'s fallback, which nothing ever overrode. That is the footer's "Version" *and* every `?v=` static cache-buster, so each deploy re-served stale CSS/JS under the same key. Wired to the `CF_VERSION_METADATA` binding PLAN.md §6507 already named as the `SOURCE_COMMIT` replacement.
> - **`csi.js` deliberately NOT loaded**, unlike Django's shell: its only admin consumer is `index.html:117`'s `outstandingtaskcount` frag, a django-tasks queue depth that §2899 already deletes. The one admin page that genuinely polls (the crawl-set detail) uses its own inline `setInterval`, ported as such.
> - `section` values corrected against Django's own: the dashboard is `"home"` (matching **no** nav item — it is reached only from the logo), not `"needs"`, which lit the wrong tab now that `/admin/needs/` is a different page.
>
> **Pages built (all verified returning 200 with real rows, and the significant ones screenshotted):** `/admin/search/` (the navbar box, six result groups, LIKE wildcards properly escaped — SQLite has no default `ESCAPE` where Postgres does); `/admin/settings/` (the hub every other page hangs off — until it existed, eight *already-built* port routes were direct-URL-only); `/admin/needs/`; `/admin/crawl-sets/` and `/admin/crawl-set/<id>/` (reusing the JSON endpoint WP 6.7 had built standalone for exactly this page); `/admin/items/` + item form; `/admin/order-groups/` + detail + form; order create/edit/delete/notify/email; `/admin/slug-redirects/` + forms; `/admin/map/`; all six `/admin/stats/*` pages; `/admin/foodbanks/dupe_postcodes/`; `/admin/foodbank/<slug>/addsub/`; photo delete; need create and need email; and `/admin/clearcache/`. Five new D1 tables came with them (`orderitem`, `ordergroup`, `slugredirect`, `placephoto`, plus order-line indexes) — migrations `0014`–`0018`, all of which PLAN.md §2987-2995 already lists as straight mechanical loads.
>
> **Two Django defects fixed rather than ported, both worth naming.** `clearcache` in Django clears two in-process `LocMemCache` dicts on whichever gunicorn worker served the request and never contacts Cloudflare at all — so the button labelled "clear the cache" left the CDN, where essentially all the cached HTML lives, untouched. It now issues a real zone purge, and is POST+CSRF rather than a bare `<a href>` GET. And the slug-redirect admin now rebuilds the KV blob the router actually reads on every save; Django's equivalent caches the map for an hour with nothing invalidating it, so a redirect added in the admin could take an hour to take effect.
>
> **Deliberately not ported, each with the evidence:** the Credentials pages (§2.9.1 deletes `GfCredential` outright — secrets are Worker secrets, and Django's `/admin/credential/<name>/` returns any secret as `text/plain`); the three testers and `needtestbed` (each needs a live third-party credential and none is reachable from Django's own nav); the geography loader endpoints (one-off/periodic data-maintenance batches, not admin pages — they map onto the existing `admin_job` + Queue pattern if ever wanted); `frag` (its one allowed slug is the django-tasks counter); and `nocalories.html`, `find_locations.html`, `foodbanks_christmascards.html` and `foodbanks_deliveryaddresses.html` — confirmed by grep to be exactly the "four orphan admin templates" §2899 already lists for deletion, with no view or URL pattern reaching any of them.
>
> **Outstanding, needing the maintainer:** `GMAP_PLACES_KEY` is not set on the account, so the two Places-backed lookup buttons return a clear 503 until it is; `CF_API_KEY`/`CF_ZONE_ID` are unset, so Clear Cache degrades to a KV-only purge and says so in a warning banner. Per this repo's own rule, none of the three has been added to `wrangler.jsonc`'s `secrets.required` — a name only goes in there in the same change that actually sets it.

> **SECURITY INCIDENT, found and closed 2026-09-02: Cloudflare was caching authenticated admin pages and serving them anonymously.** The maintainer reported `/admin/foodbank/<slug>/tab/needsorders/` returning 200 with no cookie. It was broader than that: `/admin/`, `/admin/foodbank/<slug>/` and every lazy tab fragment did too, and the subscribers tab renders subscriber identifiers, so this was a real data exposure rather than merely a wrong page.
>
> **The auth middleware was not at fault, and no change to it could have caught this.** `wrangler.jsonc` sets `"cache": { "enabled": true }`, and this section's own note on that setting says why it matters: a cache HIT is *served without executing the Worker* ("so a hit costs 0 CPU-ms"). No `/admin/*` response set `Cache-Control`, so the edge stored them; every subsequent anonymous request was answered from cache and `requireAdminAuth` never ran, because the request never reached the Worker at all. `cf-cache-status: HIT` with `age: 96` confirmed it directly. Which URLs leaked was simply which ones an admin had visited recently enough for the entry to still be live — which is why some admin URLs 302'd correctly while others served a full page.
>
> Fixed with `workers/site/src/middleware/noStore.ts` on `/admin`, `/admin/*`, `/auth` and `/auth/*`: `Cache-Control: private, no-store, max-age=0, must-revalidate`, plus `CDN-Cache-Control: no-store` and `Vary: Cookie`. Registered before the routes so it wraps every response, including the auth redirects (a cached redirect is its own, milder bug). **Four patterns rather than one**: Hono matches `"/admin/*"` against subpaths only, so the bare mount point needs its own entry or the dashboard itself stays uncovered — worth knowing before adding any future gated prefix.
>
> Remediated in three steps, each verified rather than assumed: deployed the header fix; confirmed 28 admin/auth URLs across 3 requests each now return 302 with `cf-cache-status: BYPASS` and no HIT; then ran a full zone `purge_everything` to clear any pre-fix entry still held in an edge colo the checks did not reach. Authenticated access was re-tested afterwards and still works. The purge also became the first real end-to-end exercise of the Clear Cache button (§ the WP 6.12 note above), using the `CF_API_KEY`/`CF_ZONE_ID` secrets set the same day — it returned `?cache=purged`, i.e. the Cloudflare API reported success.
>
> **The general lesson for the rest of this migration, which is not admin-specific:** on Django, `@cache_page`-style caching is opt-IN per view and an un-decorated view is private by default. Behind a Worker with the Workers Cache enabled, HTML is cacheable by DEFAULT and privacy is opt-OUT. Every response that varies by user — or that only some users may see at all — must say so explicitly. Phase 0's Cache Rules (§10.2.1) and the `Cache-Tag` work should be read with that inversion in mind: the rules there describe what to cache, and this incident is the reminder that the unlisted remainder is not automatically safe.

> **WP 6.12, fidelity sweep round 1 — 53 gaps closed, 2026-09-02.** With the ~30 missing pages from the inventory above now built, this pass went the other direction: every ported page compared against its Django original — template, view and route — each candidate gap adversarially verified before being fixed. The largest were structural content loss, not cosmetic drift: the food bank detail page's middle column had collapsed a three-cell location table down to name + postcode, losing the Main/Delivery address rows, every location's phone/email/Lat Lng/Place ID, and the Edit/Delete buttons — two already-registered routes (`/edit/address/`, location delete) were reachable from nowhere as a result. The check page was missing its whole header (website link, last-edit time, Edit/Touch/Next), stacked Ours/Found instead of side by side, and lost the per-row Delete action; its preview tabs never hid the previous iframe because the panels sat outside the `.sections` wrapper `tabber.js` scopes its hide loop to. The need page was missing Notification Preview entirely, even though the route it links to already existed. Behavioural bugs alongside the markup: Nonpertinent redirected back to the need instead of the review queue, breaking the triage loop; the orders tab sorted by `created` rather than `delivery_datetime`, listing a different 200 rows in a different order; the check job compared found donation points against the food bank's own postcode instead of its locations', and flagged phone numbers differing only in spacing as changed. Verified: typecheck and precompile clean, 15 admin URLs returning 200 locally, rendered HTML checked for the riskiest fixes.
>
> **WP 6.12, fidelity sweep round 2 — 57 more gaps closed, 2026-09-02.** Two things round 1 missed. First, a real process gap: 29 of round 1's own candidate findings had been dropped as "unverified" when their verifier agents died mid-run on a usage limit — not refuted, just never adjudicated — and they clustered almost entirely on the list pages and the edit forms, so those two surfaces had effectively never been checked. Re-adjudicated all 30; 28 held. Second, the pages WP 6.12 itself had built were reviewed for correctness against their own spec but never diffed against Django — that comparison found 29 more. **The most consequential was a sort direction:** Django applies `?sort=` raw on foodbanks, locations and donation points, so those three lists open ASCENDING — and `/admin/foodbanks/` opening oldest-edited-first is the point of the page, the same triage queue `foodbanks_next` and the dashboard's oldest-edit panel point into. The port hardcoded DESC on all three, so the food banks needing attention were on the *last* page, and locations/donation points listed Z→A. Fixed with a real direction parameter, Django's per-page sort dropdown, and a column arrow that shows the direction the list is actually in. Other behaviour bugs: saving a food bank never derived lat/long from `lat_lng` or re-slugified after a rename, so a renamed food bank kept its old slug and was invisible to postcode search under the new one; the crawl-set item table was inverted (SQLite sorts NULLs first, Postgres last), sinking the rows that actually produced a need below hundreds of blank ones; editing an order whose need fell outside the 200-row option cap silently cleared the need on save; search rejected any query over 48 characters, so the URL/push-endpoint searches Django supports returned nothing; New Need accepted an empty shopping list where Django's form rejects it; New Need's textareas had no ids, so `admin.js`'s Titlecase/CSV-to-Lines/Find & Replace buttons were never injected — the same id contract broken once already in the WP 6.12 build; Add Subscribers silently dropped duplicate pasted lines; donation point Company was free text where Django offers a select of 25 known companies; a validation failure returned a bare plain-text 400 and threw away everything typed. One more category worth naming on its own: several pages were rendering Django source citations and the phrase "matching Django" as visible admin copy — moved into nunjucks comments, which are stripped at render. Verified: typecheck and precompile clean, 42 admin URLs returning 200, the sort direction checked both ways against seeded data, the three citation-leaking pages confirmed clean.
