...
| 6.5b | ⏸️ **Not built, split out of 6.5 2026-09-02.** `Order`/`OrderItem`/`OrderGroup` forms: `Order.save()` isn't the plain 6-line stamp its siblings have — every save re-parses `items_text` through a Gemini call (`orderline_prompt.txt`, JSON-schema response) that deletes and regenerates every `OrderLine`, recomputing weight/cost/calorie aggregates from the AI's output. That's real money/data correctness riding on an AI call inside what would otherwise be a plain form save, and deserves its own careful, reviewed pass — not a few fields bolted onto this WP's pattern. Blocked further: no `ordergroup` D1 table exists yet (only `orders`/`orderline`, migrations/0005_orders_and_charity.sql — that migration's own comment already flagged "the write side... is Phase 6 scope"). `SlugRedirectForm` is deferred alongside it for an unrelated reason: no `slugredirect` D1 table, and — more importantly — no public-site read path consults one yet either, so admin CRUD for it would write data nothing serves. `PlaceForm` is deferred for a third, different reason: D1's `place` table (migrations/0009_aac.sql) was deliberately trimmed to read-only columns per §4.8.7 ("the only columns any production code path reads") — a faithful `PlaceForm` needs fields that don't exist in that schema, and un-trimming it wasn't asked for by this WP. | Order/OrderGroup/SlugRedirect/Place all editable, `ordergroup`/`slugredirect` tables exist. | 4 |
