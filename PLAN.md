...
4. **Twelve months of running two systems is more complexity, not less** — directly against goal 4. It only gets simpler at the very end.
...
6. **Key-person risk.** One of the two developers wrote the entire existing site. A half-migrated system across two platforms is materially harder to operate than either platform alone, and only the author can safely resume it.
...
- [ ] **The charity is in a period where the need-review queue cannot tolerate disruption.** That queue is the single most operationally critical workflow the charity has, and it moves last.
...
- Almost every phase is reversible by deleting a Cloudflare route.
...
**The strangler mechanism is route precedence, not proxying.** Cloudflare resolves the most specific Worker route first, and anything unmatched falls through to the Django origin exactly as it does today. So routes widen — `/needs/at/*/photo.jpg`, then `/api/*`, then `/needs/*`, then `/*` last. No `fetch()` back to Django is ever needed, which matters because a Route cannot be the target of a same-zone `fetch()`. **Rollback for any phase is deleting one route.**
...
Agree these **before starting**. Each has a checkpoint, and freezing happens at a **phase boundary**, never mid-phase — every boundary is a coherent, deployed, working state.
...
| **K4** | Phase 6 tracking > 2× estimate at its 50% mark | Mid Phase 6 | **Stop; keep Django for `/admin/` indefinitely** on a smaller box. Reopens the Cloudflare Access question the maintainer declined. |
| **K5** | An undiagnosable parity regression survives 2 days of investigation | Any phase | **Pause the phase.** Losing the ability to verify is worse than not migrating — you would be shipping blind to 3,000 food banks' data. |
| **K6** | The maintainer is unavailable > 4 weeks mid-migration | Any phase | **Freeze at the last completed boundary.** |
...
**Phases 0–6 are all reversible.** Delete the route; Django serves it again. Postgres is untouched. Rollback is measured in seconds, and the effort is not wasted — the site is faster whether or not you continue.
...
- **Before it:** delete routes, restart Django, done.
- **After it:** rollback requires a reverse sync — and that mechanism must be **built, rehearsed and timed at T-7**. Three earlier drafts of this plan described three different rollback mechanisms and the runbook implemented none of them. That gap is closed in the cutover section; do not start Phase 7 until it is.
...
**Stopping after Phase 4 is not failure.** You would have: the entire public site and all three APIs on Cloudflare; photos, maps, favicons, screenshots, dumps and static assets on R2; edge-cached HTML with working tag-based invalidation; ~3.9 GB out of a 5 GB database; and one small box serving only `/admin/`, `/dashboard/`, `/write/` and the crons.

Against the four goals at Phase 4:

- **Faster** — fully delivered.
- **Resilient** — mostly. The public site survives a box outage from cache; the admin does not.
- **Quicker deploys** — yes, for everything ported.
- **Simple** — one Django app on one box, with a Workers front end.

For a two-person charity that may well be the right place to stop. Build the plan so that stopping there is a decision, not a defeat.
...
This is the single hardest constraint on any incremental-sync design.
...
// workers/site/wrangler.jsonc
{
  ...
  // ONE route. Most-specific-wins never has to be reasoned about.
...
| Phase | HTML caching mechanism | Notes |
|---|---|---|
| Phase 0 (Django origin, no Worker route) | **Zone Cache Rules** | This is where the goal-1 win lands, before any code migration. |
| Phases 1–3 (narrow routes) | Mixed — zone Cache Rules for anything still on Django, Workers Cache for ported routes | |
| Phase 4+ (catch-all route) | **Workers Cache only** | Zone Cache Rules become irrelevant for Worker-served paths. |
...
With photos in R2 it must consult R2 (an `env.MEDIA.head()`), or better be maintained by the ingest consumer as an explicit write. **Sequence this as three deploys, not one:**

1. Backfill R2 and populate `r2_key`.
2. Deploy code that reads R2 with a DB-blob fallback and stops deriving `place_has_photo` from the live Places call.
3. Verify for a week.
4. **Only then** drop the `blob` column.

Doing steps 2 and 4 together removes the rollback path.
...
│  ├─ pg-to-d1/      # schema + data conversion
│  └─ gfdiff/        # the parity harness (two modes)
│
└─ (the existing Django tree, deleted app-by-app as each phase lands)
```
...
| **G2** | **FTS5 trigram on production D1.** `CREATE VIRTUAL TABLE … USING fts5(name_upper, tokenize='trigram')` against `--local` **and** `--remote`. Confirmed working locally; the remote check is one `wrangler d1 execute`. | 1 hour | Table creates; `MATCH '"ackne"'` returns `Hackney` and `South Hackney`; `EXPLAIN QUERY PLAN` shows no `SCAN place`. |
...
### 5.11 Delta sync and reconciliation

The full load happens at T-7d; the cutover freezes only the delta. **The delta design in
earlier drafts was wrong in a way that loses data**, and this is the corrected version.
...
delta picks up the new rows and **never removes the old ones**. That is ~4,198 duplicate
rows per day; a three-day dual-run leaves ~12,000 phantom rows and every
`/needs/at/<slug>/charity/` page renders each financial year three or four times.
...
A subscriber who signs up on day 1 and confirms on day 2 arrives in D1 with
`confirmed=False`, never receives a notification, and **nothing alerts** — the row exists,
the count is right, only the boolean is wrong.
...
2. **`CharityYear` has no `modified` column** (it is a `CreatedModel`), so any incremental delta sync keyed on `id > max_id` picks up the new rows and **never removes the old ones**. At ~4,198 rows deleted and reinserted daily, a 3-day dual-run leaves ~12,000 phantom rows and every `/needs/at/<slug>/charity/` page renders each financial year three or four times.
...
> **Task 8.7-a** — During the dual-run, `charityyear` must be **fully reloaded** in every delta cycle, not delta-synced. It is 4,198 rows / 1.4 MB; a truncate-and-reload takes seconds and removes the entire class of problem.
...
⚠ **Delta-sync hazard during dual-run.** All five extend `CreatedModel` — `created` only, **no `modified`**. But `confirmed` flips `False→True` on an existing row via the confirm link, and `last_contacted` / `last_notified` are `UPDATE`s. A watermark of `id > max_id` **cannot see any of that**.
...
The admin is in scope. The end state has no Django anywhere — no origin box kept alive for `/admin/`, no "phase 5 that never happens". This section is the build plan for `gfadmin` and `gfauth`, the D1 write paths they drive, and every security control on the new deployment.
...
Within Phase 6, build group **A first**. It is the workflow that runs every day; it deserves the longest shakedown window before the box is switched off.
...
Every line is verifiable. Run it before the route widens to `/admin/*`, and again at D+7.
...
flowchart TD
  ...
  F --> E
  E --> G[Postgres decommission<br/>unblocked]
```

Auth first because nothing else is reachable without it. Field specs second because every form depends on them. The review queue third, so it gets the longest soak.
...
| WP | Task | Acceptance criteria | pd |
|---|---|---|---|
| 6.1 | **Google OAuth in the Worker** per the maintainer's decision. Authorization-code flow, PKCE + `state` in a signed short-lived `__Host-oauth` cookie, ID-token verification against Google's JWKS (**cache by `kid`; keys rotate**), the `email_verified && hd == "givefood.org.uk"` gate from `givefood/middleware.py:68`. `/auth/receiver/` path unchanged — it is a registered redirect URI. | Existing OAuth client keeps working. A user outside the Workspace is refused. | 5 |
| 6.2 | **Sessions in KV.** Small, short-lived, read on every admin request — KV's read path is the cheapest and D1 would make a write-per-request out of every admin page. A server-side store makes logout an actual **revocation**, which signed cookies cannot do, on an admin that can mass-delete needs and notify thousands. `__Host-gfsession`, `Secure`, `HttpOnly`, **`SameSite=Lax`** (not Strict — the OAuth callback is a cross-site GET), 12h sliding. CSRF: double-submit token **plus** an `Origin`/`Sec-Fetch-Site` check. | Session survives a deploy. Logout revokes. Missing CSRF token ⇒ 403. **Known cost:** KV's ~60s eventual consistency means a logout may take up to a minute to propagate to other colos. | 4 |
| 6.3 | **Fix the security defects rather than porting them.** `GET /admin/credential/<name>/` returns any secret as `text/plain` — delete it (secrets move to Secrets Store). `GET /admin/proxy/?url=` is an unrestricted SSRF but **load-bearing** (four templates iframe it so reviewers see the source page beside the extraction) — allowlist to `Foodbank.url`/`shopping_list_url`/`locations_url`/`contacts_url`/`donation_points_url` resolved per request. Nine views mutate or spend money on GET, with `data-instant-allow-query-string` on `<body>`. | Credentials view gone. Proxy allowlisted. All mutating routes POST-only. | 3 |
| 6.4 | **Need-review queue first within the phase**: index, `/admin/need/<uuid>/`, the four transitions (publish / nonpertinent / categorise / notify), discrepancy actions. Most operationally critical workflow; give it the most shakedown time. | A need is reviewed, published (fanning out 19 translations), categorised and notified end to end, matching Django. | 8 |
...
#### 10.2.8 Phase 7 — D1 cutover · **14–20 pd** · Phase 8 — Decommission · **3–5 pd**
...
That last one is the worst. `CharityYear` is a `CreatedModel` with no `modified`, so the plan would sync it by `id > max_id`: the new rows are picked up, **the old ones are never removed from D1**. That is ~4,198 duplicate rows per day. A 3-day dual-run leaves ~12,000 phantom rows and every `/needs/at/<slug>/charity/` page renders each financial year three or four times.

**The subscriber tables are worse still**, because they are `UPDATE`s to rows with no watermark: `FoodbankSubscriber.confirmed` flips False→True via `/needs/at/<slug>/updates/confirm/`, `last_contacted` is updated by `post_to_subscriber`, `WhatsappSubscriber.last_notified` likewise. All are `CreatedModel`. A subscriber who signs up on day 1 and confirms on day 2 arrives in D1 with `confirmed=False`, **never receives a notification, and nothing alerts** — the row exists, the count looks right, only the boolean is wrong.

**The corrected delta strategy:**
...
| **K5** | Undiagnosable parity regression surviving **2 days** of investigation | Any phase | **Pause the phase.** Losing the ability to verify is worse than not migrating — you would be shipping blind to 3,000 food banks' data. |
| **K6** | Key-person loss: the maintainer unavailable >4 weeks mid-migration | Any phase | **Freeze at the last completed phase boundary.** A half-migrated system across two platforms is materially worse than either platform alone, and only the author can safely resume. |
...
**Freeze at phase boundaries, not mid-phase.** Every boundary is a coherent, deployed, working state.

#### 10.13.2 The point of no return

**Phases 0–6 are all reversible.** Delete the route; Django serves it again; Postgres is untouched. Rollback is seconds, and the effort is not wasted — the site is faster whether or not you continue.

**The point of no return is 00:50 on cutover night**, when the write freeze lifts and the first write lands in D1 that is not in Postgres.

- *Before it:* delete routes, restart Django, done.
- *After it:* rollback requires the reverse sync.
...
| >2.5×, or the `{{ obj.method }}` auto-call problem proves unautomatable | 🔴 **Descope to "Phase 2 and stop"** — APIs and R2 on Workers, HTML stays on Django. Still delivers goal 1 via Phase 0 caching. |
...
| **One runtime** | `uv run pytest && manage.py runserver` | pnpm + wrangler + a seed script — and for **~12 months, both at once** | The transition is more complexity, not less. It gets simpler only at Phase 8. |
...
**Note what is *not* on that diagram: a commitment to Phases 5–8.** They are 60% of the remaining effort and buy only the Postgres decommission. That decision belongs at the Phase 4 boundary, with eight months of actual burn data behind it — not now, on estimates.
