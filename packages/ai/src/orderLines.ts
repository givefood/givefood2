...
// The second half of givefood/models/orders.py:71-215 Order.save():
// everything below the ORM's `super().save()` -- delete the old lines, ask
// Gemini to parse items_text, write new lines, recompute the five
// aggregates, restamp the food bank's last_order.
//
// IT LIVES HERE FOR THE SAME REASON runFoodbankCheck DOES. It used to be a
// queue job in workers/jobs; the order form now runs it inline, as Django's
// save() does, because the `jobs` queue's 30 s max_batch_timeout meant an
// admin saved and then watched a banner for half a minute (the measurement
// behind github #38). workers/jobs keeps a thin wrapper so an order-lines
// message already on the queue at deploy still gets processed.
//
// Progress and outcome are still written to an admin_job row: it is what the
// order page's banner reads, and a failed parse's error message then outlives
// the redirect.
//
// Errors are recorded on the admin_job row rather than thrown, so neither
// caller retries: a retry re-runs the same paid Gemini call against the same
// input and fails the same way.
...
export interface OrderLinesTiming {
  // geminiJsonCall's own defaults (60 s retry sleep, 120 s per attempt)
  // assume a queue consumer. The order form is holding the admin's request
  // open and passes runFoodbankCheck's 2 s / 25 s instead.
  retryDelayMs?: number;
  timeoutMs?: number;
}

export async function runOrderLinesJob(db: Session, apiKey: string | undefined, jobId: string, orderRowId: number, timing: OrderLinesTiming = {}): Promise<void> {
  try {
    ...
    const raw = await geminiJsonCall({
      apiKey,
      // Django's gemini() default (givefood/utils/ai.py:17), and what
      // runFoodbankCheck uses. This was "gemini-2.0-flash", which Google has
      // retired -- every parse failed at the API with no lines written.
