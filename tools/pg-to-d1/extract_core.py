...
Usage:
    ...
    2026-09-05: foodbanksubscriber, orderitem, ordergroup, slugredirect).
...
UUID_COLUMNS = {
    ...
    "place": set(),
    "postcode": set(),
    # LAUNCH_GAP_TABLES. None carry a UUID: FoodbankSubscriber's sub_key /
    # unsub_key are 16-char CharFields (givefood/models/subscribers.py:24-25),
    # generated by the model rather than uuid.uuid4, so they copy as plain
    # text -- passing them through the UUID path would reformat values the
    # unsubscribe links in already-sent emails still have to match.
    "foodbanksubscriber": set(),
    "orderitem": set(),
    "ordergroup": set(),
    "slugredirect": set(),
}
...
# The four tables that had rows in Postgres and none in D1, found
# 2026-09-05 while auditing launch readiness -- every other table this
# script loads was already populated. Schemas match column-for-column
# (packages/db/migrations vs givefood/models/{subscribers,orders,
# operations}.py), so these are straight copies with no reshaping.
#
# foodbanksubscriber is the one that matters for a cutover: it is the
# notification list, so an empty table at the flip silently unsubscribes
# every real user. The other three are admin-visible rather than
# user-facing -- Items, Order Groups and Slug Redirects render as bare
# pages without them, and slugredirect additionally means old food bank
# URLs stop redirecting, which is an SEO cost rather than a broken page.
#
# NOT a substitute for Phase 7's own load (PLAN.md §10.2.8 WP 7.2/7.4):
# this is a snapshot, so anyone subscribing between now and the flip is
...
LAUNCH_GAP_TABLES = [
    ...
    ("givefood_ordergroup", "ordergroup", [
        'id', 'name', 'slug', 'public', 'key', 'created', 'modified',
    ], {
        # "key" is a Postgres keyword -- non-reserved, so it parses bare
        # here, but quoted for the same reason "order"/"group" are above:
        # nothing in this file should depend on which side of the
        # reserved/non-reserved line a column name happens to fall.
        "select_cols": [
            'id', 'name', 'slug', 'public', '"key"', 'created', 'modified',
        ],
    }),
...
def main():
    ...
    elif mode == "launch-gaps":
        total_loaded = run_table_list(token_box, LAUNCH_GAP_TABLES, cur)
    elif mode == "geo":
